...
// The canonical layout reads these and renders identically structured DOM.
...
const SKIN_PAGE = {
  ...
  // background paint = aged paper with foxing/candle/dust, no rule lines
  Background: () => <PageBackground/>,
  // decorative slot for the round indicator (HUD-left)
  RoundBadge: ({ round }) => (
    <div style={{
      ...
      color:'#f6dcc8', fontFamily:'"EB Garamond",serif', fontStyle:'italic', fontSize: 18, fontWeight: 700,
...
const PageBackground = () => (
  <>
    ...
    <div style={{position:'absolute', inset: 12,
      ...
      {/* foxing */}
      <div style={{position:'absolute',top:'8%',left:'12%',width:18,height:14,borderRadius:'50%',background:'radial-gradient(circle,#a87038 0%,transparent 70%)',opacity:.35}}/>
      <div style={{position:'absolute',top:'82%',right:'18%',width:24,height:18,borderRadius:'50%',background:'radial-gradient(circle,#a87038 0%,transparent 70%)',opacity:.28}}/>
      <div style={{position:'absolute',bottom:'8%',left:'8%',width:14,height:12,borderRadius:'50%',background:'radial-gradient(circle,#a87038 0%,transparent 70%)',opacity:.32}}/>
      <div style={{position:'absolute',top:'45%',right:'5%',width:10,height:9,borderRadius:'50%',background:'radial-gradient(circle,#a87038 0%,transparent 70%)',opacity:.25}}/>
      ...
      <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none',mixBlendMode:'multiply',opacity:.18}}>
        <filter id="pgN"><feTurbulence type="fractalNoise" baseFrequency=".85" numOctaves="2" seed="3"/><feColorMatrix values="0 0 0 0 0.4 0 0 0 0 0.28 0 0 0 0 0.16 0 0 0 1.2 -0.3"/></filter>
...
const PageActionSeal = ({ label, kbd, primary, warm, compact }) => {
  ...
  return (
    <button style={{
      ...
      color:'#f6dcc8', fontFamily:'"EB Garamond",serif', fontStyle:'italic',
      ...
      {kbd && !compact && <div style={{fontSize: size*0.16, opacity:.7, marginTop: 1}}>{kbd}</div>}
    </button>
  );
};
...
const SKIN_TERMINAL = {
  ...
  RoundBadge: ({ round }) => (
    <div style={{
      ...
      border: `1px solid #7fdb6a`, color: '#7fdb6a',
  ...
  ActionBtn: ({ label, kbd, primary, warm, compact }) => (
    <button style={{
      padding: compact ? '5px 8px' : '8px 14px', minWidth: compact ? 0 : 80,
      background: 'transparent',
      border: `1px solid ${primary ? '#7fdb6a' : 'rgba(127,219,106,.5)'}`,
      color: primary ? '#7fdb6a' : 'rgba(127,219,106,.85)',
      fontFamily:'"IBM Plex Mono",monospace', fontSize: compact ? 11 : 12, fontWeight: 600,
      letterSpacing:'0.08em', textTransform:'uppercase',
      cursor:'pointer', display:'flex', alignItems:'center', gap: 6,
      textShadow: primary ? `0 0 6px #7fdb6a` : 'none',
    }}>
      {kbd && <span style={{padding:'1px 5px', border:`1px solid currentColor`, fontSize: compact?9:10, fontWeight:700}}>{kbd}</span>}
      {label}
    </button>
  ),
  PathRender: 'sharp',
...
const TerminalBackground = () => (
  <>
    ...
    <div style={{position:'absolute',inset:0,
      ...
      mixBlendMode:'multiply', opacity:.7, pointerEvents:'none'}}/>
    <div style={{position:'absolute',inset:0,
      background:'radial-gradient(ellipse at center, transparent 50%, rgba(0,0,0,.55) 100%)', pointerEvents:'none'}}/>
  </>
);

// ─── FULL BLEED SKIN ──────────────────────────────────────
const SKIN_FULLBLEED = {
  id: 'fullbleed',
  name: 'Full Bleed',
  ...
  RoundBadge: ({ round }) => (
    <div style={{display:'flex',alignItems:'center',gap:8, flexShrink: 0}}>
      ...
      <span style={{fontFamily:'"IBM Plex Mono",monospace', fontSize:11, letterSpacing:'0.18em', color:'rgba(241,236,226,.55)', textTransform:'uppercase'}}>R{round}/3</span>
    </div>
  ),
  ActionBtn: ({ label, kbd, primary, warm, compact }) => (
    <button style={{
      ...
      fontFamily:'"Inter Tight",sans-serif', fontSize: compact ? 12 : 14, fontWeight: 600,
      letterSpacing:'0.02em',
      padding: compact ? '7px 12px' : '10px 20px', borderRadius: 999, cursor:'pointer',
      display:'flex', alignItems:'center', gap: 6,
    }}>
      {label}
      {kbd && !compact && <span style={{fontFamily:'"IBM Plex Mono",monospace',fontSize:9, opacity:.7, padding:'1px 4px', background: primary?'rgba(0,0,0,.18)':'rgba(255,255,255,.06)', borderRadius: 3}}>{kbd}</span>}
    </button>
  ),
...
const SKINS = { page: SKIN_PAGE, terminal: SKIN_TERMINAL, fullbleed: SKIN_FULLBLEED };

Object.assign(window, { SKINS, SKIN_PAGE, SKIN_TERMINAL, SKIN_FULLBLEED });
